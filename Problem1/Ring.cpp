...
void SetData(){
    for(int i=0;i<6*num_sides;i+=6){
        points[i] = (float)radius*cos(i*M_PI/(3*num_sides));
        points[i+1] = (float)radius*sin(i*M_PI/(3*num_sides));
        points[i+2] = 0.0f;
        points[i+3] = 1.0f;
        points[i+4] = 1.0f;
        points[i+5] = 1.0f;
...
int main(){
    ...
    points = new GLfloat[6*num_sides];
    SetData();
    ...
    glBufferData(GL_ARRAY_BUFFER, 6*num_sides*sizeof(points[0]), points, GL_STATIC_DRAW);
